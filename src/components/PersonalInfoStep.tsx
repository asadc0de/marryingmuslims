...
export const PersonalInfoStep: React.FC<PersonalInfoStepProps> = ({ data, onChange, showErrors, errors = {} }) => {
  ...
  const minDobStr = '1900-01-01';
  
  // Set maximum date to December 31, 2000
  const maxYearDobStr = '2000-12-31';
  ...
  <input
    ...
    onChange={(e) => {
      ...
      // Regex to match YYYY-MM-DD format with reasonable year range
      const datePattern = /^(19\d\d|20\d\d)-(0[1-9]|1[0-2])-(0[1-9]|[12]\d|3[01])$/;
      
      if (!inputDate || datePattern.test(inputDate)) {
        const selectedDate = new Date(inputDate);
        const minDate = new Date('1900-01-01');
        const maxDate = new Date(maxDobStr);
        
        if (!inputDate || (selectedDate >= minDate && selectedDate <= maxDate)) {
          onChange({ dateOfBirth: inputDate });
        }
      }
    }}
    min="1900-01-01"
  ...
  {/* Phone number field removed as per requirements */}

  {/* Country field removed as per requirements */}
